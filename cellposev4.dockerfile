...
RUN wget -q http://us.download.nvidia.com/tesla/418.87/NVIDIA-Linux-x86_64-418.87.00.run -O /tmp/NVIDIA-installer.run \
 ...
 && cd /tmp/dcv-inst \
 && wget -qO- https://d1uj6qtbmh3dt5.cloudfront.net/2022.2/Servers/nice-dcv-2022.2-14357-el7-x86_64.tgz |tar xfz - --strip-components=1 \
 && yum -y install \
    nice-dcv-gl-2022.2.1012-1.el7.x86_64.rpm \
    nice-dcv-server-2022.2.14357-1.el7.x86_64.rpm \
    nice-xdcv-2022.2.487-1.el7.x86_64.rpm
...
RUN source /opt/conda/etc/profile.d/conda.sh && \
    conda create --name cellpose python=3.8 -y && source /opt/conda/etc/profile.d/conda.sh && \
    conda activate cellpose && python -m pip install cellpose[gui]  

#RUN conda create --name cellpose python=3.8

#RUN conda init bash

#RUN conda activate cellpose

#RUN python -m pip install cellpose[gui]
